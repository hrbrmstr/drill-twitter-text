...
@FunctionTemplate(
  names = { "tw_extract_hashtags" },
  scope = FunctionTemplate.FunctionScope.SIMPLE,
  nulls = FunctionTemplate.NullHandling.NULL_IF_NULL
)
public class HashtagExtract implements DrillSimpleFunc {
  ...
  public void eval() {
    ...
    java.util.List<String> extracted = extractor.extractHashtags(tw_string);

    mw.startList();

    for (String tag : extracted) {

      byte[] outBytes = tag.getBytes();
